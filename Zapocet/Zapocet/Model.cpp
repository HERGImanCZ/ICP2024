...
{
	std::string text = get_file_contents(file); // Load data from file
	JSON = json::parse(text); // Parse data from loaded file
...
std::vector<unsigned char> Model::getData()
{
	...
	std::vector<unsigned char> data(bytesText.begin(), bytesText.end()); // Return text in vector
	return data;
}
...
std::vector<float> Model::getFloats(json accessor)
{
	...
	unsigned int buffViewInd = accessor.value("bufferView", 1); // Pointing to correct buffer view
	unsigned int count = accessor["count"];						// How many floats we have to get
	unsigned int accByteOffset = accessor.value("byteOffset", 0); // Buffer view start
	std::string type = accessor["type"];						// How to group floats
	...
	{
		// Storing data to temporary array
		unsigned char bytes[] = { data[i++], data[i++], data[i++], data[i++] };
		float value;
		// Transforming bytes to float
...
std::vector<GLuint> Model::getIndices(json accessor)
{
	...
	unsigned byteOffset = bufferView["byteOffset"];
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		// Preventing rereloading of textures
		bool skip = false;
		...
		if (!skip){
			// Type of texture according datas
